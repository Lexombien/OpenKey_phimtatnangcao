...
static wstring BackupClipboard() {
...
static void RestoreClipboard(const wstring& text) {
...
// Copy image file(s) to clipboard
static bool CopyImageToClipboard(const string& imagePath) {
	if (imagePath.empty()) {
		return false;
	}
	...
	for (size_t i = 0; i < imagePath.length(); ++i) {
		if (imagePath[i] == '|') {
			if (!currentPath.empty()) {
				filePaths.push_back(utf8ToWideString(currentPath));
				currentPath.clear();
			}
		} else {
			currentPath += imagePath[i];
		}
	}
	if (!currentPath.empty()) {
		filePaths.push_back(utf8ToWideString(currentPath));
	...
	// Format: DROPFILES + Path1\0 + Path2\0 + ... + \0
	size_t totalPathLen = 0;
	for (const auto& path : filePaths) {
		totalPathLen += path.length() + 1; // +1 for null terminator
	}
	totalPathLen += 1; // +1 for final null terminator

	size_t allocSize = sizeof(dropFiles) + totalPathLen * sizeof(wchar_t);

	// Allocate memory
	HGLOBAL hDrop = GlobalAlloc(GMEM_MOVEABLE, allocSize);
	if (!hDrop) {
		CloseClipboard();
		return false;
	}

	void* pDrop = GlobalLock(hDrop);
	if (!pDrop) {
		GlobalFree(hDrop);
		CloseClipboard();
		return false;
	...
	memcpy(pDrop, &dropFiles, sizeof(dropFiles));
	
	// Copy file paths
	wchar_t* pszFiles = (wchar_t*)((BYTE*)pDrop + sizeof(dropFiles));
	for (const auto& path : filePaths) {
		wcscpy_s(pszFiles, allocSize, path.c_str()); // Use safe copy
		pszFiles += path.length() + 1;
	}
...
static void SimulatePaste() {
	...
	keyEvent[0].ki.wVk = VK_LCONTROL;
	keyEvent[0].ki.dwFlags = 0;

	// Key down V
	keyEvent[1].type = INPUT_KEYBOARD;
	keyEvent[1].ki.wVk = 'V';
	keyEvent[1].ki.dwFlags = 0;

	// Key up V
	keyEvent[2].type = INPUT_KEYBOARD;
	keyEvent[2].ki.wVk = 'V';
	keyEvent[2].ki.dwFlags = KEYEVENTF_KEYUP;

	// Key up Ctrl
	keyEvent[3].type = INPUT_KEYBOARD;
	keyEvent[3].ki.wVk = VK_LCONTROL;
	keyEvent[3].ki.dwFlags = KEYEVENTF_KEYUP;
...
static void handleMacro() {
	...
	// Check if this macro has image
	string imagePath;
	int macroType = 0;
	
	if (getMacroData(pData->macroTextKey, imagePath, macroType) && macroType == 1 && !imagePath.empty()) {
		// Send macro text first
		...
		// Backup clipboard text
		wstring backupText = BackupClipboard();

		// Copy image to clipboard
		if (CopyImageToClipboard(imagePath)) {
			// Simulate Ctrl+V to paste image
			SimulatePaste();
