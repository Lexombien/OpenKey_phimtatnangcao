...
void vEnglishMode(const vKeyEventState& state, const Uint16& data, const bool& isCaps, const bool& otherControlKey) {
    ...
    if (state == vKeyEventState::MouseDown || (otherControlKey && !isCaps)) {
        ...
        if (!_hasHandledMacro && findMacro(hMacroKey, hMacroData)) {
            ...
            // Convert macroKey back to text for image lookup
            string macroText;
            for (size_t i = 0; i < hMacroKey.size(); i++) {
                Uint16 ch = keyCodeToCharacter(hMacroKey[i] & ~CAPS_MASK);
                if (ch == 0) {
                    // It's a unicode code, just try to append as is
                    macroText += (char)(hMacroKey[i] & 0xFF);
                } else {
                    macroText += (char)ch;
                }
            }
            HookState.macroTextKey = macroText;
        }
        hMacroKey.clear();
        _willTempOffEngine = false;
