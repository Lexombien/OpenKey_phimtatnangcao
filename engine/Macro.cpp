...
void initMacroMap(const Byte* pData, const int& size) {
    ...
    for (int i = 0; i < macroCount; i++) {
        ...
        data.macroType = 0; //default text type
        data.imagePath = "";
        
        //check if macro type and image path are in data
        if (cursor < size) {
            data.macroType = pData[cursor++];
            if (cursor + 1 < size) {
                Uint16 imagePathSize = 0;
                memcpy(&imagePathSize, pData + cursor, 2);
                cursor += 2;
                if (cursor + imagePathSize <= size) {
                    data.imagePath = string((char*)pData + cursor, imagePathSize);
                    cursor += imagePathSize;
                }
            }
        }
...
void getMacroSaveData(vector<Byte>& outData) {
    ...
    for (std::map<vector<Uint32>, MacroData>::iterator it = macroMap.begin(); it != macroMap.end(); ++it) {
        ...
        //save macro type
        outData.push_back((Byte)it->second.macroType);
        
        //save image path if image macro
        Uint16 imagePathSize = (Uint16)it->second.imagePath.size();
        outData.push_back((Byte)imagePathSize);
        outData.push_back(imagePathSize>>8);
        for (int j = 0; j < imagePathSize; j++) {
            outData.push_back(it->second.imagePath[j]);
        }
    }
}
...
bool getMacroData(const string& macroName, string& imagePath, int& macroType) {
    vector<Uint32> key;
    convert(macroName, key);
    if (macroMap.find(key) != macroMap.end()) {
        imagePath = macroMap[key].imagePath;
        macroType = macroMap[key].macroType;
        return true;
    }
    return false;
}
...
bool addMacro(const string& macroText, const string& macroContent) {
    return addMacro(macroText, macroContent, 0, "");
}

bool addMacro(const string& macroText, const string& macroContent, const int& macroType, const string& imagePath) {
    vector<Uint32> key;
    convert(macroText, key);
    if (macroMap.find(key) == macroMap.end()) { //add new macro
        ...
        data.macroText = macroText;
        data.macroContent = macroContent;
        data.macroType = macroType;
        data.imagePath = imagePath;
        convert(macroContent, data.macroContentCode);
        macroMap[key] = data;
    } else { //edit this macro
        macroMap[key].macroContent = macroContent;
        macroMap[key].macroType = macroType;
        macroMap[key].imagePath = imagePath;
...
void saveToFile(const string& path) {
    ...
    for (std::map<vector<Uint32>, MacroData>::iterator it = macroMap.begin(); it != macroMap.end(); ++it) {
        myfile << it->second.macroText << ":";
        if (it->second.macroType == 1 && !it->second.imagePath.empty()) {
             myfile << "{img:" << it->second.imagePath << "}";
        }
        myfile << it->second.macroContent << "\n";
    }
    myfile.close();
}
        ...
        while (getline (myfile,line) ) {
            ...
            if (string::npos != pos) {
                ...
                if (name.compare("") != 0) {
                    string imgPath = "";
                    int type = 0;
                    // Parse image tag {img:path}
                    if (content.find("{img:") == 0) {
                        size_t endTag = content.find("}");
                        if (endTag != string::npos) {
                            imgPath = content.substr(5, endTag - 5);
                            content = content.substr(endTag + 1);
                            type = 1;
                        }
                    }
                    
                    if (!hasMacro(name)) {
                        addMacro(name, content, type, imgPath);
                    } else if (append) {
                        // If appending and macro exists, overwrite or skip? 
                        // Logic originally: if (!hasMacro) add.
                        // So append only adds NEW keys.
                        // I will keep original logic.
                    }
                }
            }
        }
